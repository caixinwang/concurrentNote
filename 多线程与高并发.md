...
有了上面的一堆基础的性能池--三个。我们要提供一个便捷的方法，能够让咱们的这个程序员能够很轻易的去用它，而不需要了解里边，它应该是个什么东西。我们就引入了executor，就是个现成的工具类。它就定义了一些快速实现线程池的方法，那么何为为快速实现呢？也就是我只要知道这个线程池需要实现一个什么东西来满足我当前的业务即可，我就定义一堆实现方法。看看你里面使用cache的、还是schedule的、还是fork的线程池等等。executor里边其实底层都是这三个基础线程池类，所以我们根本不需要看executor，我们把三个基础的线程池弄明白了，自然就知道怎么操纵了。

阿里巴巴手册里面严格讲，不允许用executors创，因为这个时候会有一些神奇的问题，就是很多程序员他并不知道这底层发生了什么事，就一顿new，然后这个时候导致了这个业务异常。所以说定义线程池在实际编写业务过程中，在写公司源码的时候，在获取公司的业务代码的时候，一定要清楚这个线程池是它到底是用来干什么的，你的具体的业务场景是什么？然后你自己去new这几个线程池，并且我熟知里面的参数是干啥的，然后才能够根据具体的业务，然后具体实现里边的传参。
...
看下一个接口。刚才executor如果我说它是抽象出我要执行一个Runnable，就是我要执行一个动作，那我并不知道怎么执行，我抽象出它。那么下面这个接口又抽象了什么出来。我们把executor service切成两块，一块叫executor，一块叫service。那么这个时候呢，就相当于executor定义了，我执行体就是runnable对象。而service定义了我这个东西应该围绕着我这个execuor提供什么样的功能？提供这些功能的这个东西它就叫服务了。
...
发现并没实现execute方法。JAVA里边，我一个抽象类可以不实现从上面继承接口的抽象，而是留给我的子类去实现，也就是ThreadExecutePool来实现。

<img src="image/image-20230411171223394.png" alt="image-20230411171223394" style="zoom:80%;" />

既然execute还是不知道，那它定义了什么？定义了submit，我提交了一个动作，定义了基础模板方法，进行了判空，把task扔进去，然后执行execute，execute具体是什么不知道，子类去实现。也就是意味着我定义了一个算法，我提交一个task，我把它封装成一个RunnableFuture，然后开始执行。
...
理一下。executor里边的run方法是说我要执行一个执行体，这个执行题它怎么做我不知道，但是在AbstractExecutorService里面定义了它是个什么样的东西，是个Runnable，具体的一个类就是FutureTask。
...
来看ExecutorCompletionService类里面的submit，这里的executor是我们外部给它传进来的线程池引用。我们这个时候对newTaskFor弄出来的FutureTask进行了包装，变成了QueueingFuture。这是一个装饰者模式，将传进来的RunnableFuture包装到一个task，以便于在完成的时候done，done是一个钩子函数，可以将完成的任务放到completionQueue里面。所以说，如果线程完成了，自己会把自己放到队列里面。QueueingFuture是装饰者，加强一些功能，也就是这个done，那么在FutureTask里面是什么调用了done呢？finishCompletion调用了done，而set和cancel方法调用了finishCompletion。
...
老白！有长期工。做一个设计。线程池里面需要放线程，那么就需要一个线程容器。那么如果来的任务大于你线程容器里面的线程数量了，那么你就需要一个地方来缓存任务，也就是需要一个任务队列，但是还不够，如果任务队列也满了，我们还需要一个策略来应对新来的任务。既然线程池里面放的是线程，那么就需要一个创建线程的工厂。核心线程数是长期工而最大线程数减去核心就是临时工。
...
可以发现这个创建出来的线程不是守护线程Daemon，因为下面设置为false了。把创建出来的线程用一个原子的AtomicInteger进行编号，保证线程的编号是唯一的。传一个默认优先级给它。
...
如果你只调用了await了没调用shudown，那么什么也不会返回，因为你都没把线程池关闭，谁来触发这个关闭工作呢？如果只调用了shudown没调用await，那么说明也不会返回，因为主线程顺着运行，退出了，你这些线程也就跟着没了。
...
发现进去判空。如果当前线程数小于核心的线程数，那么直接加一个核心线程。addWorker成功就返回了，失败这里		` c = ctl.get()`重新获取了一遍，因为如果有多个线程进入到这段代码，只有一个会成功返回，另外一个应该获得此时最新的ctl。workerCountOf(c)底层就是直接用c与上 capacity(000111111....)，前面三个0，后面29个1，这里用到了我们之前说的，用一个int的高三位来表示状态，低29位表示工作线程数。
...
总结：如果线程数小于核心线程数，那么就启动一个新的核心线程执行放进来的任务。如果说核心线程数已经达到了满的情况下，把任务放到工作队列里面去。如果说工作队列已经满了的情况下，再走else if，那么没有到达最大核心线程数的时候呢，添加worker就即可。如果说addworker返回FALSE也就到达了最大线程数，放不下这个任务调拒绝函数。这里的拒绝会有两种，一种是因为线程池关闭了，一种是因为咱们的任务满了。
...
看一下这一段代码，什么意思？addWorker我们知道是接客的，添加线程的。我们知道如果一个线程池处于运行状态，那么没问题至直接接。但是如果一个线程不是运行时了，也就是SHUTDOWN往后的状态，那么我大概率不接了，但是有一种情况，SHUTDOWN情况其实可能会接的，所以我们要判断一下。

如果if成立，那么我们就接。也就是说如果rs >= SHUTDOWN了，我们还要判断一下接不接。也就是说								`(rs == SHUTDOWN &&firstTask == null &&! workQueue.isEmpty())`这一段要为true我们才接。接客说明我们要跳出这个if，不能进入到里面。所以接客的条件就是rs为shutdown并且不是第一个任务并且队列不为空我们才接。
